...
const DepartureFilter: React.FC = () => {
  ...
  const departureStatus = useSelector(
    (state: RootState) => state.flightFilterReducer.departureStatus,
  );
  ...
  const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (isChecked) {
      dispatch({
        ...
        payload: [...departureStatus, value],
      });
    } else {
      dispatch({
        type: 'departureStatus',
        payload: departureStatus.filter(
        ...
        return (
          <div className="form-check collection-filter-checkbox" key={index}>
            <input
              ...
              id={data.type}
              value={data.type}
              checked={departureStatus.includes(data.type || '')}
