...
const HotelFilter: FC = () => {
  ...
  return (
    <div className="filter-block">
      <div
        className={`collection-collapse-block border-bottom-0 ${show ? 'open' : ''}`}
      >
        <h6 className="collapse-block-title" onClick={() => setShow(!show)}>
          hotels
        </h6>
        <div
          className={`collection-collapse-block-content pb-0${show ? 'd-none' : ''} `}
        >
          <div className="sidebar-hotels">
            {HotelDetailFilter.map((item: IHotelFilterProps, index: any) => (
