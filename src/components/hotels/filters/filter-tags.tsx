...
const FilterTags: FC = () => {
  ...
  const filterTourTags = useSelector(
    (state: RootState) => state.tourFilterReducer,
  );
  const flightFilterTag = useSelector(
    (state: RootState) => state.flightFilterReducer,
  );
  const cabFilterTag = useSelector(
    (state: RootState) => state.cabFilterReducer,
  );
  ...
  const removeFilter = (key: string, value: string) => {
    if (key === 'price') {
      ...
      if (key in filterTags) {
        dispatch({
          type: key,
          payload: filterTags[key].filter((val: string) => val !== value),
        });
      } else if (key in filterTourTags) {
        dispatch({
          type: key,
          payload: filterTourTags[key].filter((val: string) => val !== value),
        });
      } else if (key in flightFilterTag) {
        dispatch({
          type: key,
          payload: flightFilterTag[key].filter((val: string) => val !== value),
        });
      } else if (key in cabFilterTag) {
        dispatch({
          type: key,
          payload: cabFilterTag[key].filter((val: string) => val !== value),
        });
      }
    }
  };
  ...
  const allTags: [string, string][] = [
    ...
    ...Object.entries(filterTourTags),
    ...Object.entries(flightFilterTag),
    ...Object.entries(cabFilterTag),
  ];
    ...
    <ul className="product-filter-tags">
      {allTags.map(([key, value]) => (
        <li className="me-2" key={key}>
          {Array.isArray(value)
            ? value.map((item: any, index) => renderTag(key, item, index))
            : renderTag(key, value)}
        </li>
      ))}
    </ul>
