...
const SkeletonGridView: FC = () => {
  const { gridSize, gridStyle } = useSelector(
    (state: RootState) => state.gridReducer,
  );
  ...
  const renderProducts = () => {
    ...
    for (let i = 0; i < numProducts; i++) {
      products.push(
        <div
          key={i}
          className={`${gridSize === 3 && 'col-xl-4'} ${gridSize === 4 && 'col-xl-3 col-lg-4'} col-sm-6 grid-item`}
        >
  ...
  return (
    <div className="product-wrapper-grid special-section grid-box">
      {gridStyle === 'list-view' ? (
        ...
        <div className={'row content grid'}>{renderProducts()}</div>
      )}
    </div>
  );
};
