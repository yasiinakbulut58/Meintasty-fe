...
const useFilterFlight = ({ value }: IFilterProductsProps) => {
  ...
  const { stopStatus, departureStatus, arriveStatus, airlineStatus } =
    useSelector((state: RootState) => state.flightFilterReducer);
  ...
  const createQueryString = useCallback(
    (params: URLSearchParams, name: string, values: string[]) => {
      params.delete(name);
  ...
  useEffect(() => {
    const filteredProducts = value?.filter((product: IBaseProps) => {
      ...
      const filterStops =
        stopStatus.length === 0 ||
        (typeof product.stop === 'string' && stopStatus.includes(product.stop));
      const filterAirline =
        airlineStatus.length === 0 ||
        (typeof product.airline === 'string' &&
          (airlineStatus.includes(product.airline) ||
            airlineStatus.includes('all')));
      const filterArrive =
        arriveStatus.length === 0 ||
        (typeof product.cdg === 'string' && arriveStatus.includes(product.cdg));
      const filterDeparture =
        departureStatus.length === 0 ||
        (typeof product.dxb === 'string' &&
          departureStatus.includes(product.dxb));
    ...
    ['stop', 'dxb', 'cdg', 'price', 'airline'].forEach((name) =>
      params.delete(name),
    );
