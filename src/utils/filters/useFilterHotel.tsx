  ...
  useEffect(() => {
    const filteredProducts = value?.filter((product: IBaseProps) => {
      ...
      const filteredDistrict =
        ...
        (product.district !== undefined &&
          districtStatus.includes(product.district));
      const filteredFacility =
        facilityStatus.length === 0 ||
        product.facility === undefined ||
        facilityStatus.includes(product.facility);
      const filteredHostLang =
        hostLangStatus.length === 0 ||
        product.language === undefined ||
        hostLangStatus.some((lang: string) => lang === product.language);
      const filteredRate =
        rateStatus.length === 0 ||
    ...
    ['district', 'facility', 'language', 'rate', 'price'].forEach((name) =>
      params.delete(name),
    );
