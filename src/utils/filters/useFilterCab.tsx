...
const useFilterCab = ({ value }: IFilterProductsProps) => {
  ...
  const { cabTypeStatus, cabOptionStatus, capacityStatus } = useSelector(
  ...
  useEffect(() => {
    const filteredProducts = value?.filter((product: IBaseProps) => {
      ...
      const filteredRate =
        rateStatus.length === 0 ||
        (product.rate !== undefined && rateStatus.includes(product.rate));
      const filterCabType =
        cabTypeStatus.length === 0 ||
        (product.cabType !== undefined &&
          cabTypeStatus.includes(product.cabType));
      const filterCabOption =
        cabOptionStatus.length === 0 ||
        !Array.isArray(product.option) ||
        cabOptionStatus.some((option) => product.option?.includes(option));
      ...
      const filteredRanges =
        ...
        capacityStatus.some((range) => {
          ...
          return (
            ...
            product.capacity >= minCapacity &&
            product.capacity <= maxCapacity
          );
        });
